"""
Fine-tune and evaluate ChromatogramCNN on the MTBLS288 rice cultivar dataset.

Usage:
  python scripts/mtbls288_finetune_evaluate.py
  python scripts/mtbls288_finetune_evaluate.py --device cuda:0
  python scripts/mtbls288_finetune_evaluate.py --config configs/finetune_rice.yaml

Results are written to results/mtbls288/chroma_results.json
"""
...
from chroma_dcnn.downstream.rice import load_rice_chroma_paths
...
def main(config_path: str, device: str | None = None) -> None:
    ...
    if groups is not None:
        print(f'Grouped CV — {len(set(groups))} biological replicates')
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(description='Evaluate ChromatogramCNN on MTBLS288 rice GC-MS')
