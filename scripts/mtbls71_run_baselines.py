"""
Run baselines (PLS-DA, RF, SVM, MLP) on the MTBLS71 urine dataset.
...
Usage:
  python scripts/10_run_baselines_urine.py
  python scripts/10_run_baselines_urine.py --config configs/finetune_urine.yaml

Results are written to results/mtbls71/baseline_gcms_{representation}_results.json
"""
...
def main(config_path: str, grouped_cv: bool = False) -> None:
    ...
    cv_strategy = "grouped" if grouped_cv else cfg["task"].get("cv_strategy", "kfold")
    cv_folds = cfg["task"].get("cv_folds", 5)
    data_dir = Path(cfg["data"]["data_dir"])

    from chroma_dcnn.downstream.urine import load_urine_data, load_urine_chroma_features

    X_sum, y, _, _ = load_urine_data(data_dir)
    chroma_features, _ = load_urine_chroma_features(data_dir)
    ...
    if cv_strategy == "grouped" and groups is not None:
        print(f"Using grouped CV — {len(set(groups))} donor groups")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Run baselines on MTBLS71 urine GC-MS")
    ap.add_argument("--config", default="configs/finetune_urine.yaml")
    ap.add_argument("--grouped-cv", action="store_true",
                    help="Use donor-grouped CV to prevent replicate leakage")
    args = ap.parse_args()
    main(args.config, grouped_cv=args.grouped_cv)
