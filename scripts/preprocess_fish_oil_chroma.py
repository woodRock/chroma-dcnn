"""
Preprocess fish oil GC-MS CSVs into uniform-RT-binned chromatograms for ChromatogramCNN.

For each sample:
  1. Read full 2D chromatogram (4802 scans × 344 m/z channels)
  2. Map m/z channels to 1000-bin grid
  3. Divide into N equal-width RT windows (~24 scans each at N=200)
...
Preserves the RT axis (unlike preprocess_fish_oil_scans.py which ranks by TIC),
so ChromatogramCNN can exploit chromatographic peak structure with 1D convolutions.

Output: data/fish_oil/chroma/{sample_id}.npz
  chroma : float32 [n_bins, 1000]

Usage:
  python scripts/preprocess_fish_oil_chroma.py
  python scripts/preprocess_fish_oil_chroma.py --n-bins 200
"""
...
def _read_mz_channels(filepath: Path) -> list[int]:
    with open(filepath, encoding="utf-8") as fh:
        fh.readline()
        row1 = fh.readline()
    return [int(float(p.strip())) for p in row1.split(",")[2:] if p.strip().replace(".", "").isdigit()]
...
def _bin_chromatogram(spectra: np.ndarray, tic: np.ndarray, n_bins: int) -> np.ndarray:
    ...
    Returns [n_bins, mz_max] — zero rows where the window has no signal.
    """
    n_scans = len(tic)
...
def process_file(filepath: Path, mz_channels: list[int], n_bins: int) -> np.ndarray:
    ...
    chroma = _bin_chromatogram(spectra, tic, n_bins)   # [n_bins, MZ_MAX]
    chroma = _sqrt_l2(chroma)                           # normalise each bin
    return chroma
