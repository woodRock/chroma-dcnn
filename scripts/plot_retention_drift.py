...
before and after alignment, with matched peak pairs annotated to make the drift
magnitude visible.

Usage:
  python scripts/plot_retention_drift.py
  python scripts/plot_retention_drift.py --pair SNA1_Gonad   # default
  python scripts/plot_retention_drift.py --pair SNA1_Liver
...
def align_cow(query: np.ndarray, ref: np.ndarray,
    ...
    def seg_corr(r_seg, q_start, q_len):
        ...
        if len(q_seg) != len(r_seg):
            q_seg = np.interp(np.linspace(0,1,len(r_seg)), np.linspace(0,1,len(q_seg)), q_seg)
    ...
    warp = interp1d(ref_pts, q_pts, kind='linear', bounds_error=False, fill_value='extrapolate')
...
def make_figure(pair_key: str, out_path: Path) -> None:
    ...
    # L2-normalise TICs for visual comparison
    ...
    for ax, tic_ref, tic_qry, r_val, subtitle, pk_pairs, show_drift in panel_data:
        ax.plot(TIME_AX, tic_ref, color=COLOURS[0], lw=1.4,
        ...
        # Annotate matched peak drift with vertical dashed lines + text
        if show_drift:
            plotted = 0
            # Sort pairs by drift magnitude descending; take top 6
            ...
            for pa, pb in sorted_pairs:
                ...
                # Vertical dashed lines at each peak position
                ax.axvline(ta, ymin=0, ymax=1, color=COLOURS[0],
                           lw=0.6, ls='--', alpha=0.45, zorder=1)
                ax.axvline(tb, ymin=0, ymax=1, color=COLOURS[1],
                           lw=0.6, ls='--', alpha=0.45, zorder=1)

                # Horizontal bracket line connecting the two dashed lines
...
def main() -> None:
    ...
    ap.add_argument('--pair', default='SNA1_Gonad',
