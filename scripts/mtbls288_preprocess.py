"""
Download and preprocess MTBLS288 rice grain GC-MS data for ChromatogramCNN evaluation.
...
Groups (for grouped CV): biological replicate number (G-1 … G-4)
  Each replicate is a different plant measured across all 5 time points;
  all samples from the same plant go to the same CV fold to avoid leakage.

Note: developmental stage (7/10/14/28/42 DAF) is a nuisance variable —
  metabolome changes substantially across stages, making this a harder task
  than stage-homogeneous datasets.
...
Output (data/mtbls288/):
  raw/           raw .cdf files (~5 GB; skipped if already present)
...
Usage:
  ...
  python scripts/mtbls288_preprocess.py --workers 8
  python scripts/mtbls288_preprocess.py --no-download   # process already-downloaded files
"""
...
def _get_filenames() -> list[str]:
    """Fetch CDF filenames from the EBI FTP index page."""
...
def _download(fname: str, dest: Path) -> None:
    """Download one CDF file; skip silently if already present."""
    if dest.exists():
        return
...
def _build_cdf_to_meta(assay_rows: list[dict], sample_rows: list[dict]) -> dict[str, dict]:
    """
    Returns {cdf_stem: {cultivar, stage, group, label}} for each non-blank sample.

    cdf_stem: e.g. '13242ob_1'
    """
    sample_meta = {}
    ...
    for row in assay_rows:
        ...
        if label is None:
            continue
        # Group = biological replicate number extracted from sample name (e.g. "G-1")
...
def main(raw_dir: Path, output_dir: Path, n_bins: int, workers: int, no_download: bool) -> None:
    ...
    if not no_download:
        ...
        print(f"Downloading ({workers} parallel workers) — skipping files already present …")
