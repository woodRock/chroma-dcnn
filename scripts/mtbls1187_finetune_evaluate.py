"""
Fine-tune and evaluate ChromatogramCNN on the MTBLS1187 wheat era dataset.

Usage:
  python scripts/mtbls1187_finetune_evaluate.py
  python scripts/mtbls1187_finetune_evaluate.py --device cuda:0
  python scripts/mtbls1187_finetune_evaluate.py --config configs/finetune_wheat.yaml

Results are written to results/mtbls1187/chroma_results.json
"""
...
from chroma_dcnn.downstream.wheat import load_wheat_chroma_paths
...
def main(config_path: str, device: str | None = None) -> None:
    ...
    if groups is not None:
        print(f"Grouped CV — {len(set(groups))} cultivars")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Evaluate ChromatogramCNN on MTBLS1187 wheat GC-QTOF")
